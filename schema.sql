-- ============================================================
-- AgroColetivo — Schema v4 (DROP + CREATE COMPLETO)
...
-- ── users: login de pivôs e vendors ───────────────────────────
create table users (
  id            uuid        primary key default gen_random_uuid(),
  name          text        not null,
  phone         text        unique not null,   -- usado como login
...
-- ── buyers: fazendeiros (acesso via link, sem senha) ───────────
create table buyers (
  id         uuid        primary key default gen_random_uuid(),
  name       text        not null,
...
-- ── vendors: fornecedores (vinculados a um user com role=vendor)
...
-- ── product_promotions: promoções condicionais ─────────────────
...
-- ── campaigns: cotações criadas por pivôs ─────────────────────
...
-- ── campaign_lots: ofertas de fornecedores numa cotação ────────
