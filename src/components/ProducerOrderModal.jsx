...
export function ProducerOrderModal({ campaign, onClose, onSave }) {
  ...
  const qtyNum  = +qty
  const tons    = qty ? ((qtyNum * (campaign.unitWeight ?? 25)) / 1000).toFixed(2) : null
  const qtyOk   = qtyNum >= minQty && (maxQty === null || qtyNum <= maxQty)
  const canSave = name.trim().length > 1 && unmaskPhone(phone).length >= 10 && qtyOk
  ...
  const handlePhone = async (e) => {
    ...
    if (raw.length >= 10) {
      ...
      if (found) { setName(found.name); setReturning(found) }
    }
  }
      ...
      <ModalBody>
        ...
        <div className="form-group">
          ...
          <input className="form-input" placeholder="(38) 99123-4567"
            value={phone} onChange={handlePhone} inputMode="tel" autoFocus/>
          {returning && (
            <div style={{
              marginTop:6, padding:'6px 10px',
              background:'var(--primary-dim)', border:'1px solid var(--primary-border)',
              borderRadius:'var(--r)', fontSize:'.75rem', color:'var(--primary)',
        ...
        <div className="form-group">
          <label className="form-label">
            ...
            Quantidade ({campaign.unit}){minMsg}{maxMsg}
          </label>
          <input type="number" className="form-input"
            placeholder={`Mín. ${minQty}${maxQty !== null ? ` · Máx. ${maxQty}` : ''}`}
            value={qty} onChange={e => setQty(e.target.value)}
            inputMode="numeric" min={minQty} max={maxQty ?? undefined}/>
        ...
        <div style={{
          ...
          fontSize:'.78rem', color:'var(--text2)', lineHeight:1.5,
        }}>
          <strong style={{color:'var(--blue)'}}>Retorno automático</strong> — Digite o WhatsApp para verificar se o produtor já está cadastrado. Um mesmo produtor pode participar de várias cotações.
        </div>
      </ModalBody>
      <ModalFooter>
