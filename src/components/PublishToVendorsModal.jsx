...
export function PublishToVendorsModal({
  campaign,
  vendors,
  onPublish,
  onClose,
  ...
  const handlePublish = async () => {
    console.log("🖱️ Botão Publicar clicado");
    if (selected.size === 0) {
      console.warn("❌ Nenhum vendor selecionado");
      return;
    }
    setPublishing(true);
    try {
      console.log("📤 Chamando onPublish callback...");
      await onPublish(campaign.id);
      console.log("✅ onPublish concluído");
      ...
      console.log(
        `📧 Enviando emails para ${selectedVendorsList.length} vendors...`,
      );
      ...
      for (const vendor of selectedVendorsList) {
        ...
        if (vendor.email) {
          try {
            console.log(`📧 Enviando para ${vendor.name} (${vendor.email})...`);
            const result = await notifyVendorNewProposal(
              vendor.email,
              vendor.name,
              {
            ...
            if (result?.success) {
              console.log(`✅ Email enviado para ${vendor.name}`);
            } else {
              console.warn(
                `⚠️ Falha ao enviar email para ${vendor.name}:`,
                result?.error,
              );
            }
          } catch (err) {
            console.warn(`⚠️ Erro ao enviar email para ${vendor.name}:`, err);
          }
        } else {
        ...
        {!published ? (
          <>
            ...
            >
              ...
              . Após publicar, você enviará a mensagem individualmente por
              WhatsApp.
            </p>
            ...
            {vendors.length === 0 ? (
              ...
              >
                ...
                />
                <span>
                  Você não tem nenhum fornecedor cadastrado. Vá em{" "}
                  <strong>Fornecedores</strong> para adicionar antes de
                  publicar.
                </span>
              </div>
            ) : (
              <>
                ...
                >
                  ...
                  >
                    {selected.size} de {vendors.length} selecionado
                    {vendors.length !== 1 ? "s" : ""}
                  </span>
          ...
          <>
            ...
            >
              <Check size={15} /> Cotação publicada! Agora envie para cada
            ...
            >
              Clique em <strong>Enviar</strong> para abrir o WhatsApp com a
              mensagem já preenchida para cada fornecedor.
            </p>
        ...
        {!published ? (
          <>
            ...
            <Button
              variant="primary"
              disabled={
                selected.size === 0 || publishing || vendors.length === 0
              }
            ...
            >
              {publishing ? (
                ...
                <>
                  <Send size={14} /> Publicar para {selected.size} fornecedor
                  {selected.size !== 1 ? "es" : ""}
                </>
              )}
            </Button>
