...
export function useCampaigns(user) {
  ...
  const reloadCampaign = useCallback(async (campaignId) => {
    ...
    setCampaigns(prev => prev.map(c => {
      ...
      const totalSupplied = lots.reduce((s, l) => s + (l.qtyAvailable ?? 0), 0)
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      if (user.role === ROLES.VENDOR && !user.vendorId) {
        ...
        if (!vRow && user.phone) {
          const { data: vByPhone } = await supabase
            ...
            .maybeSingle()
          if (vByPhone) {
            await supabase.from('vendors').update({ user_id: user.id }).eq('id', vByPhone.id)
            vRow = vByPhone
          }
        }
      ...
      const withOrders = rawCampaigns.map(c => {
        ...
        const visibleLots   = isVendor && vendorId ? lots.filter(l => l.vendorId === vendorId) : lots
        const totalSupplied = lots.reduce((s, l) => s + (l.qtyAvailable ?? 0), 0)
        const approved = orders.filter(o => o.status === 'approved').map(normalizeOrder)
        const pending  = orders.filter(o => o.status === 'pending').map(normalizeOrder)
  ...
  return {
    campaigns, vendors, ownVendor, loading, error,
    reload: loadAll, reloadCampaign,
    addCampaign, saveFinancials,
