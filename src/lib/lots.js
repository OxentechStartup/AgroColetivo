...
export async function createLot(campaignId, lot) {
  const { data, error } = await supabase
    ...
    .insert({
      ...
      freight:        Number(lot.freight  || 0),
      markup:         Number(lot.markup   || 0),
...
export async function assignOrderToLot(orderId, lotId) {
  const { error } = await supabase
    ...
    .update({ lot_id: lotId })
    .eq('id', orderId)
  if (error) throw new Error('Erro ao alocar produtor: ' + error.message)
}
...
export async function fetchOrdersWithLots(campaignId) {
  const { data, error } = await supabase
    ...
    .select(`
      ...
      producers (id, name, phone),
      campaign_lots (id, vendor_name, price_per_unit, freight, markup, qty_available, vendors(name))
    `)
    .eq('campaign_id', campaignId)
...
function normalizeLot(r) {
  const vendorLabel = r.vendors?.name || r.vendor_name || 'Fornecedor avulso'
  return {
    id:           r.id,
