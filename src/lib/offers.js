...
export async function createOffer(campaignId, vendorId, offer) {
  ...
  const { data, error } = await supabase
    ...
    .select()
    .single()
  if (error) throw new Error('Erro ao enviar oferta: ' + error.message)
 ...
 */
export async function settleOffersAfterAccept(campaignId, acceptedOfferId, acceptedQty) {
  ...
  const { data: orders, error: ordErr } = await supabase
    ...
    .eq('campaign_id', campaignId)
    .eq('status', 'approved')
  if (ordErr) throw new Error('Erro ao buscar pedidos: ' + ordErr.message)
  ...
  const { data: lots, error: lotErr } = await supabase
    ...
    .eq('campaign_id', campaignId)
  if (lotErr) throw new Error('Erro ao buscar lotes: ' + lotErr.message)
  const totalSupplied = (lots ?? []).reduce((s, l) => s + Number(l.qty_available), 0)
  ...
  if (pendErr) throw new Error('Erro ao buscar propostas pendentes: ' + pendErr.message)
...
export async function fetchVendorOffers(vendorId) {
  ...
  const { data: offers, error } = await supabase
    .from('vendor_campaign_offers')
    .select('id, campaign_id, vendor_id, price_per_unit, available_qty, notes, status, created_at')
    .eq('vendor_id', vendorId)
    .order('created_at', { ascending: false })
  if (error) throw new Error('Erro ao buscar suas propostas: ' + error.message)
...
export async function cancelAcceptedOffer(offerId, campaignId, vendorId) {
  ...
  const { data: remainingLots } = await supabase
    ...
    .select('qty_available')
    .eq('campaign_id', campaignId)
  const stillSupplied = (remainingLots ?? []).reduce((s, l) => s + Number(l.qty_available), 0)
