...
export async function addPromotion(productId, promo) {
  const { data, error } = await supabase
    ...
    .select()
    .single()
  if (error) throw new Error('Erro ao criar promoção: ' + error.message)
...
export async function removePromotion(promoId) {
  const { error } = await supabase
    ...
    .delete()
    .eq('id', promoId)
  if (error) throw new Error('Erro ao remover promoção: ' + error.message)
}
...
export function promoLabel(promo) {
  ...
  if (promo.promoType === 'percent_discount') return `${promo.value}% de desconto (${trigger})`
  if (promo.promoType === 'fixed_discount')   return `R$ ${promo.value.toFixed(2)} de desconto (${trigger})`
  if (promo.promoType === 'fixed_bonus')      return `Bônus de R$ ${promo.value.toFixed(2)} (${trigger})`
