...
export async function upsertVendorProduct(vendorId, product) {
  ...
  const pricePerUnit =
    product.pricePerUnit != null ? Number(product.pricePerUnit) : null;
  const pricePerKg =
    product.pricePerKg != null ? Number(product.pricePerKg) : null;

  const resolvedPrice =
    pricePerUnit ??
    (pricePerKg && weightKg ? +(pricePerKg * weightKg).toFixed(2) : null);
  ...
  if (product.id) {
    const { data, error } = await supabase
      ...
      .select()
      .single();
    if (error) throw new Error(error?.message || "Erro ao atualizar produto");
...
export async function softDeleteVendorProduct(id) {
  const { error } = await supabase
    ...
    .update({ active: false })
    .eq("id", id);
  if (error) throw new Error(error?.message || "Erro ao deletar produto");
}
...
export async function addPromotion(productId, promo) {
  const promoType =
    promo.discountType === "percent" ? "percent_discount" : "fixed_discount";
  const { data, error } = await supabase
    ...
    .select()
    .single();
  if (error) throw new Error(error?.message || "Erro ao criar promoção");
  return data;
}
...
export async function deletePromotion(id) {
  const { error } = await supabase
    ...
    .delete()
    .eq("id", id);
  if (error) throw new Error(error?.message || "Erro ao deletar promoção");
}
...
export function calcDiscountedPrice(basePrice, qty, promotions = []) {
  const eligible = promotions
    ...
    .map((p) => {
      const disc =
        p.promo_type === "percent_discount" || p.discount_type === "percent"
  ...
  if (!eligible.length)
    return { finalPrice: basePrice, discount: 0, promoUsed: null };
  const best = eligible[0];
  return {
    finalPrice: Math.max(0, basePrice - best.discountPerUnit),
