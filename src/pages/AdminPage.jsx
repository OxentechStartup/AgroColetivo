...
const fmtPhone = (p) =>
  p ? p.replace(/(\d{2})(\d{2})(\d{4,5})(\d{4})/, "($1) $2 $3-$4") : "—";
...
function GestorRow({ gestor, campaigns, onToggle, toggling }) {
  ...
  const gestorCampaigns = campaigns.filter(
    (c) => c.pivoId === gestor.id && c.status !== "finished",
  );
  const totalFee = gestorCampaigns.reduce(
    (s, c) => s + calcPlatformFee(campaignRealValue(c), 1.5).feeValue,
    0,
  );
  ...
  return (
    <div
      className={`${styles.gestorCard} ${isBlocked ? styles.gestorBlocked : ""}`}
    >
      <div className={styles.gestorRow}>
        <div
          className={`${styles.gestorStatusDot} ${isBlocked ? styles.dotBlocked : styles.dotActive}`}
        />
        <div className={styles.gestorInfo}>
          <div className={styles.gestorName}>{gestor.name}</div>
          <div className={styles.gestorMeta}>
            <span>
              <Phone size={10} /> {fmtPhone(gestor.phone)}
            </span>
            {gestor.city && (
              <span>
                <MapPin size={10} /> {gestor.city}
              </span>
            )}
        ...
        <div className={styles.gestorStats}>
          <div className={styles.gestorStat}>
            <span className={styles.gestorStatVal}>
              {gestorCampaigns.length}
            </span>
            <span className={styles.gestorStatLbl}>cotações</span>
          </div>
          <div className={styles.gestorStat}>
            <span
              className={styles.gestorStatVal}
        ...
        <div className={styles.gestorActions}>
          ...
          {gestorCampaigns.length > 0 && (
            <button
              className={styles.expandGestorBtn}
      ...
      {expanded && (
        <div className={styles.gestorCampaigns}>
          {gestorCampaigns.map((c, i) => {
            ...
            const pct =
              c.goalQty > 0
                ? Math.min(100, Math.round((ordered / c.goalQty) * 100))
                : 0;
            return (
...
export function AdminPage({ campaigns, actions, reload }) {
  ...
  const loadGestors = useCallback(async () => {
    setGestorLoad(true);
    try {
      setGestors(await fetchGestorsAdmin());
    } catch (e) {
      showToast(e?.message || "Erro ao carregar gestores", "error");
    } finally {
      setGestorLoad(false);
    }
  }, [showToast]);
  ...
  const openCamps = allValues.filter(
  ...
  const uniqueProducers = new Set(
    campaigns.flatMap((c) => c.orders.map((o) => o.producerName)),
  ).size;
  const uniqueVendors = new Set(
    campaigns.flatMap((c) => (c.lots ?? []).map((l) => l.vendorName)),
  ).size;
  const totalOrders_n = campaigns.reduce(
    (s, c) => s + c.orders.length + (c.pendingOrders?.length ?? 0),
    0,
  );

  const blockedGestors = gestors.filter((p) => !p.active);
  const filteredGestors = gestors.filter(
    (p) =>
