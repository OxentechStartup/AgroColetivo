        ...
        {priceNum > 0 && +qty > 0 && (
          ...
          >
            <span style={{ color: "var(--text2)" }}>
              {vendorName || "Fornecedor"}: {qty} {unit}
            </span>
            {" = "}
...
function StepLots({ campaign, stats, onAdd, onRemove, onDone }) {
  ...
  return (
    <div
      className={`${styles.step} ${done ? styles.stepDone : styles.stepActive}`}
    >
      <div className={styles.stepHead}>
        <div className={styles.stepNum}>{done ? "✓" : "1"}</div>
        <div className={styles.stepInfo}>
          <div className={styles.stepTitle}>
            <Package size={14} /> Fornecedor escolhido
          </div>
          <div className={styles.stepDesc}>
            Proposta aceita que vai abastecer a demanda
          </div>
        </div>
      ...
      {(lots.length > 0 || totalOrdered > 0) && (
        <div
          className={`${styles.coverageBar} ${stats.isFulfilled ? styles.coverageOk : lots.length > 0 ? styles.coverageWarn : styles.coverageNeutral}`}
        >
          {stats.isFulfilled ? (
            <>
              <CheckCircle2 size={14} /> Demanda coberta —{" "}
              {stats.totalAvailable} {unit} disponíveis
            </>
          ) : lots.length > 0 ? (
            <>
              ...
              {Math.max(
                0,
                (campaign.orders?.reduce((s, o) => s + o.qty, 0) ?? 0) -
      ...
      {lots.length === 0 ? (
        <div className={styles.emptyStep}>
          ...
          <p
            style={{ fontSize: ".76rem", marginTop: 4, color: "var(--text3)" }}
          >
            Aceite uma proposta na aba "Propostas" da cotação, ou adicione
            manualmente acima.
          </p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                <div className={styles.lotCardHead}>
                  <div>
                    {idx === 0 && (
                      <div className={styles.lotBadge}>★ Principal</div>
                    )}
                    <div className={styles.lotCardName}>{lot.vendorName}</div>
                    {lot.notes && (
                      <div className={styles.lotCardNote}>{lot.notes}</div>
                    )}
                  </div>
                  <button
                    className={styles.delBtn}
                    onClick={() => onRemove(campaign.id, lot.id)}
                  >
                    <Trash2 size={13} />
                  </button>
                </div>
                <div className={styles.lotStats}>
                  <div>
                    <span className={styles.lotLabel}>Qtd</span>
                    <span className={styles.lotVal}>
                  ...
                  <div>
                    <span className={styles.lotLabel}>Subtotal</span>
                    <span className={styles.lotVal} style={{ fontWeight: 700 }}>
                      {formatCurrency(lot.qtyAvailable * lot.pricePerUnit)}
                    </span>
                  </div>
                  {totalOrdered > 0 && (
                    <div>
                      <span className={styles.lotLabel}>Uso</span>
                      <span className={styles.lotVal}>
                ...
                {totalOrdered > 0 && (
                  <div className={styles.lotBar}>
                    <div
                      className={styles.lotBarFill}
                      style={{
                        width: `${usedPct}%`,
                        background:
                          usedPct >= 100 ? "var(--amber)" : "var(--primary)",
                      }}
                    />
                  </div>
                )}
...
function StepFreight({ campaign, stats, onSave, onDone }) {
  ...
  return (
    <div
      className={`${styles.step} ${hasFreight ? styles.stepDone : styles.stepActive}`}
    >
      <div className={styles.stepHead}>
        <div className={styles.stepNum}>{hasFreight ? "✓" : "2"}</div>
        <div className={styles.stepInfo}>
          <div className={styles.stepTitle}>
            <Truck size={14} /> Frete & Taxa do gestor
          </div>
          <div className={styles.stepDesc}>
            Divididos igualmente entre os compradores
          </div>
        </div>
        <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
          <Button
            variant={hasFreight ? "outline" : "secondary"}
            size="sm"
      ...
      {hasFreight ? (
        <div className={styles.freightGrid}>
          <div className={styles.freightItem}>
            <span className={styles.freightLabel}>
              <Truck size={11} /> Frete total
            </span>
            <span className={styles.freightVal}>
              {formatCurrency(campaign.freightTotal ?? 0)}
            </span>
            <span className={styles.freightSub}>
              {formatCurrency((campaign.freightTotal ?? 0) / n)}/comprador
            </span>
          </div>
          <div className={styles.freightItem}>
            <span className={styles.freightLabel}>
              <TrendingUp size={11} /> Taxa do gestor
            </span>
            <span className={styles.freightVal}>
              {formatCurrency(campaign.markupTotal ?? 0)}
            </span>
            <span className={styles.freightSub}>
              {formatCurrency((campaign.markupTotal ?? 0) / n)}/comprador
            </span>
          </div>
          <div
            className={styles.freightItem}
            style={{ background: "var(--surface3)" }}
          >
            <span className={styles.freightLabel}>Total encargos</span>
            <span
              className={styles.freightVal}
              style={{ color: "var(--text)" }}
            >
              {formatCurrency(
                (campaign.freightTotal ?? 0) + (campaign.markupTotal ?? 0),
              )}
            </span>
            <span className={styles.freightSub}>
              {formatCurrency(
        ...
        <div className={styles.emptyStep}>
          <p style={{ color: "var(--text3)", fontSize: ".84rem" }}>
            Nenhum frete ou taxa definido ainda. Clique em "Definir frete" ou
            avance sem.
          </p>
        </div>
      )}
...
function StepBuyers({ campaign, stats }) {
  ...
  const buildMsg = (o, forUrl = false) => {
    ...
    return (
      ...
      (encargosEach > 0
        ? `• Frete + taxas: ${formatCurrency(encargosEach)}${nl}`
        : "") +
  ...
  const sendWa = (o) => {
    ...
    window.open(
      `https://wa.me/${phone ? "55" + phone : ""}?text=${buildMsg(o, true)}`,
      "_blank",
    );
  };
  ...
  return (
    <div
      className={`${styles.step} ${isReady ? styles.stepActive : styles.stepLocked}`}
    >
      <div className={styles.stepHead}>
        <div className={styles.stepNum}>3</div>
        <div className={styles.stepInfo}>
          <div className={styles.stepTitle}>
            <Users size={14} /> Custo por Comprador
          </div>
          <div className={styles.stepDesc}>
            {isReady
      ...
      {!isReady ? (
        <div className={styles.emptyStep}>
          <p style={{ color: "var(--text3)", fontSize: ".84rem" }}>
            Complete a etapa 1 para ver o custo de cada comprador.
          </p>
        </div>
      ) : (
        <>
            ...
            <table className="tbl">
              <thead>
                <tr>
                  ...
                  <th>Total</th>
                  <th style={{ width: 90 }}>Notificar</th>
                </tr>
              </thead>
              <tbody>
                {orders.map((o) => {
              ...
              return (
                <div key={o.orderId} className={styles.buyerCard}>
                  <div className={styles.buyerCardHead}>
                    ...
                    <strong className={styles.buyerTotal}>
                      {formatCurrency(total)}
                    </strong>
                  </div>
                  <div className={styles.buyerRows}>
                    <div className={styles.buyerRow}>
                      <span>
                        {o.qty} {unit} × {formatCurrency(stats.avgPrice)}
                      </span>
...
function SummaryBar({ campaign, stats }) {
  ...
  const items = [
    ...
    {
      label: "Total geral",
      value: stats.totalGross > 0 ? formatCurrency(stats.totalGross) : "—",
      icon: DollarSign,
      primary: true,
    },
  ];
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            ...
            <p className="text-muted">
              Custo final por comprador com frete e taxas
            </p>
          </div>
        </div>
      </div>
      ...
      {financialCampaigns.length === 0 ? (
        <div className={styles.emptyPage}>
          ...
          <p style={{ fontWeight: 600, color: "var(--text2)" }}>
            Nenhuma cotação disponível para o financeiro.
          </p>
          <p
            style={{ fontSize: ".82rem", color: "var(--text3)", marginTop: 4 }}
          >
            Aceite uma proposta em "Cotações → Propostas" para liberar o
            financeiro.
          </p>
        </div>
      ) : !active ? (
        <div className={styles.emptyPage}>
          <DollarSign size={36} style={{ opacity: 0.2 }} />
          <p>Selecione uma cotação acima.</p>
        </div>
      ) : (
        <div className={styles.content}>
            ...
            {[
              {
                n: 1,
                label: "Fornecedor",
                done: lots.length > 0 && stats.avgPrice > 0,
              },
              {
                n: 2,
                label: "Frete & Taxa",
                done:
                  (active.freightTotal ?? 0) > 0 ||
                  (active.markupTotal ?? 0) > 0,
              },
          ...
          {step === 3 && <StepBuyers campaign={active} stats={stats} />}
        </div>
      )}
